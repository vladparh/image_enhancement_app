    ...
    def __init__(
        ...
        if initialize:
            ...
            self.dec_lo = nn.Parameter(
                torch.rand_like(dec_lo) * 2 - 1, requires_grad=True
            )
            self.dec_hi = nn.Parameter(
                torch.rand_like(dec_hi) * 2 - 1, requires_grad=True
            )
            self.rec_lo = nn.Parameter(
                torch.rand_like(rec_lo) * 2 - 1, requires_grad=True
            )
            self.rec_hi = nn.Parameter(
                torch.rand_like(rec_hi) * 2 - 1, requires_grad=True
            )
        ...
        if self.use_sa:
            self.sa_h = nn.Sequential(
                ...
                nn.Conv2d(
                    dim // 4, 1, kernel_size=1, padding=0, stride=1, bias=True
                ),  # c -> 1
            )
        ...
        if self.use_ca:
            self.ca_h = nn.Sequential(
                ...
                nn.Conv2d(
                    dim, dim, 1, padding=0, stride=1, groups=1, bias=True
                ),  # conv2d
            )
...
class DWT(nn.Module):
    ...
    def forward(self, x):
        ...
        for _ in range(self.level):
            ...
            wavelet_component.append(
                (
                    lh_component.squeeze(2),
                    hl_component.squeeze(2),
                    hh_component.squeeze(2),
                )
            )
...
class IDWT(nn.Module):
    ...
    def forward(self, x, weight=None):
        ...
        for c_pos, component_lh_hl_hh in enumerate(x[1:]):
            ...
            l_component = F.conv_transpose2d(
                l_component, idwt_kernel, stride=2, groups=c
            )
...
class FFT3(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv_in(x)
        x_patch = rearrange(
            x, "b c (h patch1) (w patch2) -> b c h w patch1 patch2", patch1=8, patch2=8
        )
